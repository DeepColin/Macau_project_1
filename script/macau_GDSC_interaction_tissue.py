...
result_folder = origin_path+'MACAU_PROJECT/DATA_RESULT_STORAGE/TISSUE_SPECIFIC_GDSC/'+ name_of_drug_feature+'_'+name_of_cell_feature+'/run_'+ str(random.randint(1, 1000))
result_folder_origin = origin_path+'MACAU_PROJECT/DATA_RESULT_STORAGE/TISSUE_SPECIFIC_GDSC/'+ name_of_drug_feature+'_'+name_of_cell_feature
...
if len(all_runs) >= 20:  # if you are using this to compute functional similarity between 2 protein targets, consider usign more than 40 runs   
    for i in range( 0 , len(prefixed) ): 
        ...
        for file in all_runs:
            interaction = pd.read_csv(file+'/'+'interaction_'+prefixed[i]+'_'+name_of_drug_feature+'_'+name_of_cell_feature +'_sample'+str(nsamples)+'_latent'+str(num_latent)+'.csv',index_col = False) 
            interaction = interaction.drop(interaction.columns[0], axis=1)
            interaction_store.append(interaction)
    
        interaction_mean = pd.concat([each.stack() for each in interaction_store],axis=1).apply(lambda x:x.mean(),axis=1).unstack()     

        result_folder_MEAN = result_folder_origin + '/MEAN'
        if os.path.exists(result_folder_MEAN):
            os.chdir(result_folder_MEAN)
        else:
            os.makedirs(result_folder_MEAN) ; os.chdir(result_folder_MEAN)
